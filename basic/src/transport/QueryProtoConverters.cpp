...
namespace QueryProtoConverters
{
    ...
    QueryRequest fromProtoSubmitQueryRequest(const mini2::query::SubmitQueryRequest &in)
    {
        ...
        out.scanRowBudget = in.scan_row_budget();
        out.entryNodeId = "";
        out.originNodeId = "";
        out.distributedAllowed = true;

        // Restores visited-node state from the client request if present.
        for (const auto &nodeId : in.visited_node_ids())
        {
            out.visitedNodeIds.push_back(nodeId);
        }
    ...
    QueryRequest fromProtoSubmitSubQueryRequest(const mini2::query::SubmitSubQueryRequest &in)
    {
        ...
        // Allows subqueries to continue forwarding through the configured directed overlay.
        out.distributedAllowed = true;

        // Restores visited-node state so shared overlay nodes are not queried twice.
        for (const auto &nodeId : in.visited_node_ids())
        {
            out.visitedNodeIds.push_back(nodeId);
        }
